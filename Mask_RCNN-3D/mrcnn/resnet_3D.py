...
def identity_block(input_shape, filters, stage, block, kernel_size=3, \
    ...
    x = Conv3D(filters=filt1, kernel_size=1, strides=strides, \
...
def conv_block(input_shape, filters, stage, block, kernel_size=3, \
               strides=2, use_bias=True, train_bn=True):
...
class ResNet(object):
    def build(input_shape, architecture, stage5=False, train_bn=True):
        ...
        x = Conv3D(filters=64, kernel_size=7, strides=2, name='conv1', use_bias=True)(x)
        x = BatchNorm(name='bn_conv1')(x, training=train_bn)
        x = Activation('relu')(x)
        C1 = x = MaxPooling3D(pool_size=3, strides=2, padding='same')(x)

        # Stage 2
        x = conv_block(x, filters=[64, 64, 256], stage=2, block='a',strides=1,train_bn=train_bn)
