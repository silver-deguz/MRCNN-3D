...
from keras.layers import Input, Lambda, Activation, Add

from modelutils_3D import norm_boxes
from model_layers_3D import RPN_model
...
class MaskRCNN():
    ...
    def build(self, mode, config):
        ...
        if mode == "training":
            ...
            gt_boxes = Lambda(lambda x: norm_boxes(
            ...
            if config.USE_MINI_MASK:
                input_gt_masks = Input(
                    shape=[config.MINI_MASK_SHAPE[0], config.MINI_MASK_SHAPE[1],
        ...
        if callable(config.BACKBONE):
            ...
            _, C2, C3, C4, C5 = ResNet101(input_image, stage5=True,
                                          train_bn=config.TRAIN_BN)
        ...
        # RPN Model
        rpn = RPN_model(config.RPN_ANCHOR_STRIDE, len(config.RPN_ANCHOR_RATIOS),
        ...
        if mode == "training":
            ...
            if not config.USE_RPN_ROIS:
                ...
                # Normalize coordinates
                target_rois = KL.Lambda(lambda x: norm_boxes(
